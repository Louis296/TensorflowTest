...
from SeedCup.model import My_MSE_loss
import torch.backends.cudnn as cudnn
from SeedCup.dataLoader import TrainSet,ValSet
from SeedCup.model import Network
...
def train(epoch):
    ...
    optimizer=torch.optim.SGD(net.parameters(),lr=opt.get_lr(epoch))
    for batch_idx, (inputs, targets_sign_day, targets_sign_hour, targets_ship_day, targets_ship_hour, targets_got_day, targets_got_hour, targets_dlved_day, targets_dlved_hour) in enumerate(tqdm(trainloader)):
        ...
        if batch_idx == 1:
            print("==> epoch {}: loss_day is {}, loss_hour is {} ".format(epoch, loss_1_1, loss_1_2))
...
def val(epoch):
    ...
    for batch_idx, (inputs, payed_time, signed_time) in enumerate(tqdm(valloader)):
        ...
        for i in range(len(inputs)):
            ...
            if pred_time_hour>18 or pred_time_hour<12:
                continue
    ...
    if onTimePercent_result>0.96:
        torch.save(net, opt.TEMP_MODEL_SAVE_PATH+str(onTimePercent_result)+'-'+str(rankScore_result)+".pkl")
